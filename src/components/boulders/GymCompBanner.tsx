  ...
  return (
    <div className="bg-gradient-to-r from-amber-500/15 via-slate-900 to-slate-900 border border-amber-500/30 hover:border-amber-500/50 rounded-2xl overflow-hidden transition-all shadow-md">
      ...
      <div className="p-3 sm:p-3.5 flex items-center justify-between gap-3">
        ...
        >
          <div className="w-8 h-8 rounded-xl bg-amber-400 text-black flex items-center justify-center font-black shrink-0 shadow-md shadow-amber-400/20 group-hover:scale-105 transition-transform">
            <Trophy className="w-4 h-4 text-slate-950 stroke-[2.5]" />
          </div>

          <div className="min-w-0">
            <div className="flex items-center gap-1.5">
              <span className="text-xs font-black text-amber-300 tracking-tight flex items-center gap-1 group-hover:text-amber-200 transition-colors">
                <span>{gym?.name || 'Gym'} Comp Standings</span>
              </span>
              <span className="text-[10px] px-1.5 py-0.2 bg-amber-400/20 text-amber-400 font-mono font-bold rounded-md border border-amber-400/30">
                ACTIVE
              </span>
            </div>

            {/* Quick text summary for mobile */}
            <div className="text-[11px] text-slate-300 truncate mt-0.5 flex items-center gap-2">
              {topThree[0] && (
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          <button
            ...
            className="p-1.5 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800 transition-colors text-xs font-semibold flex items-center gap-1"
          ...
          <button
            type="button"
            onClick={onOpenFullLeaderboard}
            className="flex items-center gap-1 px-2.5 py-1.5 rounded-xl text-xs font-bold bg-amber-400 hover:bg-amber-300 text-black active-press shadow-sm transition-all"
          >
      ...
      {isExpanded && (
        <div className="p-3 sm:p-3.5 pt-0 border-t border-amber-500/20 mt-1 bg-slate-950/50 flex flex-col gap-2.5 animate-in slide-in-from-top-1 duration-150">
            ...
            {topThree.map((standing, index) => {
              ...
              return (
                <div
                  ...
                  onClick={onOpenFullLeaderboard}
                  className={`p-2.5 rounded-xl flex flex-col items-center gap-1 cursor-pointer transition-all ${
                    isGold
                ...
                >
                  <div className="relative">
                    <span
                      className={`w-4 h-4 rounded-full flex items-center justify-center font-mono text-[9px] font-black absolute -top-1 -left-1 ${
                        isGold
                  ...
                  <span className="text-[9px] font-mono text-slate-400">
                    {standing.topsCount} tops • {standing.flashesCount}⚡
                  </span>
                </div>
              );
            })}
          ...
          <div className="flex items-center justify-between text-[11px] pt-1 text-slate-400 font-mono">
            ...
            >
              <span>View Full Scorecards</span>
